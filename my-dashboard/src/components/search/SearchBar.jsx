...
function SearchBar({ onSearch, loading, resultOrder, resultLineItems }) {
  ...
  const canSearch = useMemo(
    () => supportedFields.some((field) => values[field.param]?.trim()),
  ...
  const derivedValues = useMemo(() => {
    ...
    return {
      ...
      sku: normalizeDisplayValue(firstLine?.imPartNbr ?? firstLine?.sku),

      transactionId: normalizeDisplayValue(
        resultOrder?.xactSet ?? resultOrder?.transactionId,
      ),

      poNumber: normalizeDisplayValue(resultOrder?.poNumber),

      countryCode: normalizeDisplayValue(resultOrder?.countryCode),
    };
  ...
  }, [values, canSearch, loading, onSearch, supportedFields]);
